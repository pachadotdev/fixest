...
#'
#' To use leads/lags of variables in the estimation, you can: i) either provide the argument `panel.id`, ii) either set your data set as a panel with the function [`panel`], [`f`][fixest2::l] and [`d`][fixest2::l].
#'
...
#'
#' You can use the functions [`setFixest_vcov`] and [`setFixest_ssc`][fixest2::ssc] to permanently set the way the standard-errors are computed.
#'
...
feols <- function(fml, data, vcov, weights, offset, subset, split, fsplit, split.keep, split.drop,
  ...
  if (do_multi_lhs || do_multi_rhs) {
    ...
    if (do_multi_rhs) {
      ...
      rhs_group <- list(1)
      rhs <- list(0)
      rhs_col_id <- list(seq_len(NCOL(X)))
