...
// [[Rcpp::export]]
List cpp_cholesky(NumericMatrix X, double tol = 1.0 / 100000.0 / 100000.0,
                  int nthreads = 1) {
...
void set_sparse(std::vector<int> &n_j, std::vector<int> &start_j,
                ...
                const NumericMatrix &X, const NumericVector &w) {
...
void mp_sparse_XtX(NumericMatrix &XtX, const std::vector<int> &n_j,
                   const std::vector<int> &start_j,
                   const std::vector<int> &all_i, const std::vector<double> &x,
                   const NumericMatrix &X, int nthreads) {
...
void mp_sparse_Xty(NumericVector &Xty, const std::vector<int> &start_j,
                   const std::vector<int> &all_i, const std::vector<double> &x,
...
// mp: mat prod
void mp_XtX(NumericMatrix &XtX, const NumericMatrix &X, const NumericMatrix &wX,
            int nthreads) {
...
// mp: mat prod
void mp_Xty(NumericVector &Xty, const NumericMatrix &X, const double *y,
            int nthreads) {
...
List cpp_sparse_products(NumericMatrix X, NumericVector w, SEXP y,
  ...
  if (sparse_check(X) == false) {
    ...
    //
    // } else {
    //     // Identique, mais pas besoin de faire une copie de X ni de y qui
    //     peuvent etre couteuses
    //
    //     // XtX
...
// [[Rcpp::export]]
NumericMatrix cpp_mat_reconstruct(NumericMatrix X,
                                  Rcpp::LogicalVector id_excl) {
...
// mp: mat prod
void mp_ZXtZX(NumericMatrix &ZXtZX, const NumericMatrix &XtX,
...
// mp: mat prod
void mp_ZXtu(NumericVector &ZXtu, const NumericMatrix &X,
...
void mp_sparse_ZXtZX(NumericMatrix &ZXtZX, const NumericMatrix &XtX,
                     const std::vector<int> &n_j,
                     const std::vector<int> &start_j,
                     const std::vector<int> &all_i,
                     const std::vector<double> &x, const NumericMatrix &X,
                     const NumericMatrix &Z, const NumericMatrix &wZ,
                     int nthreads) {
...
void mp_sparse_ZXtu(NumericVector &ZXtu, const std::vector<int> &start_j,
                    const std::vector<int> &all_i, const std::vector<double> &x,
                    const double *u, const NumericMatrix &X,
                    const NumericMatrix &wZ, int nthreads) {
...
List cpp_iv_products(NumericMatrix X, SEXP y, NumericMatrix Z, SEXP u,
  ...
  for (int v = 0; v < n_vars_u; ++v) {
    NumericVector ZXtu_tmp(K2 + K1);
    mp_sparse_ZXtu(ZXtu_tmp, start_j, all_i, x, REAL(VECTOR_ELT(u, v)), X, wZ,
                   nthreads);
    ZXtu[v] = ZXtu_tmp;
  }
...
// [[Rcpp::export]]
List cpp_iv_product_completion(NumericMatrix XtX, NumericVector Xty,
                               NumericMatrix X, NumericVector y,
                               NumericMatrix U, NumericVector w, int nthreads) {
  // We compute the following
  // - (UX)'(UX)
  // - (UX)'y
