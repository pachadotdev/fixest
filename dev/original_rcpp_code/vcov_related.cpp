...
#pragma omp parallel for num_threads(nthreads) schedule(static, 1)
    ...
    // I still don't know how to conveniently do the same with
    // Rcpp::NumericMatrix...
    int K_sq = K * K;
...
// [[Rcpp::export]]
NumericMatrix cpp_newey_west_panel(NumericMatrix S, NumericVector w,
                                   IntegerVector unit, int G,
...
// [[Rcpp::export]]
NumericMatrix cpp_vcov_conley(NumericMatrix S, NumericVector lon_rad,
                              NumericVector lat_rad, const int distance,
  ...
  // - using only the latitude is the way to go, even though the data is usually
  // more scattered across longitude and it would save time to use longitude as
  // the main cutoff, that would imply just too many computations
  //
  // [longitude cutoff]
  // - the longitude cutoff depends on the latitude of the points
  // - I initially used only the latitude of i
  // - but there are two points with different latitudes => thus that breaks
  // symmetry!
  // - in the end I took the avg latitudes
  // - I think that in some rare instances there can still be cases where:
  //   + dist_ij < cutoff, but..
  //   + dist_lon_rad_ij > cutoff_lon_mean_ij (and possibly: dist_lon_rad_ij >
  //   cutoff_lon_i OR dist_lon_rad_ij > cutoff_lon_j)
  // - but it should be super edge cases, at the very limit... so not really
  // important
  //
  // [using longitude]
  // - given the problems mentioned above, I should NEVER use longitude as the
  // main dimension
  // - Laurent, remember you first started with longitude in main and although
  // you can't mention here all the problems that you encountered, you did
  // encounter them. So don't.
  //
...
#pragma omp parallel for num_threads(nthreads)
  for (int i = 0; i < N; ++i) {
    ...
    for (int i2 = i + 1; i2 < N; ++i2) {
      ...
      if (distance == 1) {
        dist = dist_km(lon_rad_i, lat_rad_i, cos_lat_i, lon_rad[i2],
                       lat_rad[i2], cos_lat[i2]);
        ok = dist <= cutoff;
      ...
      // if(cpt++ < 10) Rcout << "i = " << i << ", j = " << i2 << ", dij_lon = "
      // << dist_lon_rad << ", dij_lat = " << dist_lat_rad << ", d_ij = " <<
      // dist << ", w = " << weight << "\n";
