...
void extract_quote(const char *str, int &i, int n, string &operator_tmp) {
  char quote = str[i++];
  operator_tmp += quote;
...
void extract_operator(const char *str, int &i, int n,
                      vector<string> &operator_vec, bool &is_eval,
...
[[cpp11::register]] strings cpp_paste_conditional_(strings x, integers id,
                                                   int n) {
