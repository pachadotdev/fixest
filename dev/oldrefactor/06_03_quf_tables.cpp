...
void quf_table_sum_single(void *px_in, string &x_type, int n, int q, int *x_quf,
                          ...
                          vector<int> &any_pblm, vector<bool> &id_pblm,
                          bool check_pblm, bool do_refactor, int x_size,
                          integers &obs2keep) {
...
#pragma omp parallel for num_threads(nthreads)
  ...
  if (is_pblm) {
    ...
    // this works with gcc but not with clang: res.push_back({"obs_removed"_nm =
    // logicals(obs_removed)});
