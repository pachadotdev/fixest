...
void demean_single_1(int v, PARAM_DEMEAN *args) {
  ...
  // If save_fixef is true, copy the values from fe_coef to fixef_values
...
void demean_acc_2(int v, int iterMax, PARAM_DEMEAN *args) {
  ...
  // vector of FE coefs => needs not be equal to nb_coef_T (because Q can be
  // higher than 2)
  ...
  // bool isMaster = omp_get_thread_num() == 0;
  // bool *pStopNow = args->stopnow;
  // double flop = 20.0 * static_cast<double>(n_obs); // rough estimate nber
  // operation per iter
  ...
  // update the output
  FE_info.add_2_fe_coef_to_mu(p_alpha_final, p_beta_final, p_sum_in_out, output,
                              false);
...
bool demean_acc_gnl(int v, int iterMax, PARAM_DEMEAN *args) {
  ...
  // double flop = 4.0*(5 + 12*(Q-1) +
  // 4*(Q-1)*(Q-1))*static_cast<double>(n_obs); rough estimate nber operation
  // per iter int iterSecond = ceil(2000000000 / flop / 5); // nber iter per 1/5
  // second
