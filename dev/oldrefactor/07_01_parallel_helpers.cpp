...
[[cpp11::register]] list cpppar_which_na_inf_vec_(SEXP x, int nthreads = 1) {
  /*
      This function takes a vector and looks at whether it contains NA or
     infinite values return: flag for na/inf + logical vector of obs that are
     na/inf x is ALWAYS a numeric vector std::isnan, std::isinf are OK since
     cpp11 required do_any_na_inf: if high suspicion of NA present: we go
     directly constructing the vector is_na_inf in the "best" case (default
     expected), we need not construct is_na_inf
  */
...
[[cpp11::register]] list cpppar_which_na_inf_mat_(doubles_matrix<> mat,
                                                  int nthreads = 1) {
...
[[cpp11::register]] list cpppar_which_na_inf_df_(SEXP df, int nthreads = 1) {
  // almost identical to cpppar_which_na_inf_vec but for R **numeric** data
  // frames. Changes:
...
[[cpp11::register]] integers cpppar_check_only_0_(doubles_matrix<> x_mat,
                                                  int nthreads = 1) {
