...
FEClass::FEClass(int n_obs, int Q, SEXP r_weights, SEXP fe_id_list,
  ...
  if (is_weight && nb_coef_noVS_T > 0) {
    // Checking the 0-weights => we set them to 1 to avoid division by 0
...
void FEClass::add_wfe_coef_to_mu_internal(int q, double *fe_coef_C,
  ...
  if (!is_slope_Q[q]) {
    ...
    for (int i = 0; i < n_obs; ++i) {
      for (int v = 0; v < V; ++v) {
        // Calculate the out_N using VS_mat, my_vs_coef, and weights if
        // applicable
        if (use_weights) {
...
void compute_fe_gnl(double *p_fe_coef_origin, double *p_fe_coef_destination,
  ...
  for (int q = Q - 1; q >= 0; q--) {
    ...
    if (q != 0) {
      // We recompute it from scratch (only way -- otherwise precision problems
      // arise)
