...
void quf_double(int n, int *x_uf, void *px, vector<double> &x_unik,
  ...
  x_unik.push_back(is_string ? static_cast<double>(x_order[0] + 1)
                             : ull_to_float(xim1));
  ...
  for (int i = 1; i < n; ++i) {
    ...
    if (xi != xim1) {
      ++k;
      x_unik.push_back(is_string ? static_cast<double>(x_order[i] + 1)
                                 : ull_to_float(xi));
    }
    x_unclass[i] = k;
    xim1 = xi;
...
void quf_int_gnl(int n, int *x_uf, void *px, vector<double> &x_unik, int x_min,
                 bool is_double) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    x_uint_current =
        is_double ? static_cast<int>(px_dble[i] - x_min) : px_int[i] - x_min;
...
void quf_int(int n, int *x_uf, void *px, vector<double> &x_unik, int x_min,
             int max_value, bool is_double = false) {
  // Principle:
...
[[cpp11::register]] list cpp_quf_gnl(SEXP x) {
  ...
  // 2**31) we use the algorithm for double

  // DOUBLE: quf_int is the most efficient function.
  // even if we have a vector of double, we check whether the underlying
  // structure is in fact int, so that we can send it to quf_int if the vector
  // is really double, then the checking cost is negligible if the vector is in
  // fact int, the cost of checking is largely compensated by the
  //   efficiency of the algorithm

  // STRING: for string vectors, we first transform them into ULL using their
  // pointer before applying them the algorithm for doubles note that the x_unik
  // returned is NOT of type string but is equal to the location of the unique
  // elements
  ...
  return writable::list({"x_uf"_nm = r_x_uf, "x_unik"_nm = x_unik});
}
...
void quf_table_sum_single(void *px_in, std::string &x_type, int n, int q,
                          int *x_quf, vector<double> &x_unik,
                          vector<int> &x_table, double *py,
                          vector<double> &sum_y, bool do_sum_y, bool rm_0,
                          bool rm_1, bool rm_single, vector<int> &any_pblm,
                          vector<bool> &id_pblm, bool check_pblm,
                          bool do_refactor, int x_size, integers &obs2keep) {
  ...
  if ((rm_0 || rm_single) && check_pblm) {
    ...
    int d_end = 0;
    for (int d = 0; d < D; ++d) {
      if ((rm_0 && sum_y[d] == 0) || (rm_1 && sum_y[d] == x_table[d]) ||
          (rm_single && x_table[d] == 1)) {
        any_pblm[q] = 1;
        d_end = d;
        break;
      }
...
[[cpp11::register]] list cpp_quf_table_sum(SEXP x, SEXP y, bool do_sum_y,
  ...
  // When the data is refactored (ie x is a fixef_id_list, each element ranging
  // from 1 to n_items):
  // - do_refactor
  ...
  for (int q = 0; q < Q; ++q) {
    if (identical_x) {
      ...
      res_x_quf_all[q] =
          PROTECT(Rf_allocVector(INTSXP, do_refactor ? n_keep : n));
...
#pragma omp parallel for num_threads(nthreads)
  for (int q = 0; q < Q; ++q) {
    quf_table_sum_single(px_all[q], x_type_all[q], n, q, p_x_quf_all[q],
                         x_unik_all[q], x_table_all[q], py, sum_y_all[q],
                         do_sum_y, rm_0, rm_1, rm_single, any_pblm,
                         id_pblm_all[q], check_pblm[q], do_refactor,
                         px_sizes[q], obs2keep);
  }
...
#pragma omp parallel for num_threads(nthreads)
    for (int q = 0; q < Q; ++q) {
      quf_refactor_table_sum_single(
          n, p_x_quf_all[q], p_x_new_quf_all[q], obs_removed, x_unik_all[q],
          x_new_unik_all[q], x_removed_all[q], x_table_all[q], py, sum_y_all[q],
          do_sum_y, rm_1, rm_single, id_pblm_all[q], check_pblm[q], pstop_now,
          Q);
    }
  ...
  for (int q = 0; q < Q; ++q) {
    res_tmp.push_back(doubles(x_unik_all[q]));
  }
  ...
  for (int q = 0; q < Q; ++q) {
    res_tmp2.push_back(integers(x_table_all[q]));
  }
  ...
  for (int q = 0; q < Q; ++q) {
    if (do_sum_y) {
      copy_sum_y_all.push_back(doubles(sum_y_all[q]));
    } else {
