...
void quf_table_sum_single(void *px_in, std::string &x_type, int n, int q,
                          int *x_quf, std::vector<double> &x_unik,
                          std::vector<int> &x_table, double *py,
                          std::vector<double> &sum_y, bool do_sum_y, bool rm_0,
                          bool rm_1, bool rm_single, std::vector<int> &any_pblm,
                          std::vector<bool> &id_pblm, bool check_pblm,
...
[[cpp11::register]] list cpp_quf_table_sum_(SEXP x, SEXP y, bool do_sum_y,
  ...
  std::vector<int> x_sizes_fake(Q, 0);
  ...
  std::vector<std::vector<int>> x_table_all(Q);
  std::vector<std::vector<double>> x_unik_all(Q);
  std::vector<int> any_pblm(Q, 0);
  std::vector<std::vector<bool>> id_pblm_all(Q);
  // The following may not be needed:
  std::vector<std::vector<double>> sum_y_all(Q);
  std::vector<bool> obs_removed;
  std::vector<std::vector<double>> x_removed_all(Q);
  ...
  std::vector<void *> px_all(Q);
  std::vector<std::string> x_type_all(Q);
  // vector to store modified strings
  std::vector<std::vector<unsigned long long>> x_ull_all(Q);
...
#pragma omp parallel for num_threads(nthreads)
  ...
  if (is_pblm) {
    ...
    if (is_parallel) {
      ...
      std::vector<int> obs_removed_int(n, 0);

#pragma omp parallel for num_threads(nthreads)
      for (int q = 0; q < Q; ++q) {
        if (any_pblm[q]) {
          std::vector<bool> &id_pblm = id_pblm_all[q];
