...
// Problematic function, in Rcpp it was (with Rcpp::export in [[ ]] in front)
...
//       Rprintf(
//           "Problem getting FE, maximum iterations reached (2nd order
//           loop).");
//     }
//     if (nb2do_next == 0) break;
//   }
//   if (iter == iterMax) {
//     Rprintf("Problem getting FE, maximum iterations reached (1st order
//     loop).");
//   }
//   List res(Q + 1);
//   int *pindex;
...
[[cpp11::register]] list cpp_get_fe_gnl_(int Q, int N, doubles sumFE,
  ...
  for (int q = 0; q < Q; q++) {
    // rcpp: writable::integers tableCluster(cluster_sizes(q));
    // cpp11: we cannot use () for vectors, we use []
