...
panel_setup <- function(data, panel.id, time.step = NULL, duplicate.method = "none",
  ...
  res <- list(
    order_it = order_it, order_inv = order_inv, id_sorted = id_sorted,
...
lag.formula <- function(x, k = 1, data, time.step = NULL, fill = NA,
  ...
  if (length(qui_pblm) > 0) {
    stopi(
      "In the formula the variable{$s, enum.q, is ? qui_pblm} not in the ",
  ...
  meta_info <- panel_setup(data,
    panel.id = x, time.step = time.step,
...
panel <- function(data, panel.id, time.step = NULL, duplicate.method = c("none", "first")) {
  ...
  meta_info <- panel_setup(data,
    panel.id = panel.id, time.step = time.step,
...
"[.fixest_panel" <- function(x, i, j, ...) {
  ...
  if (!missing(i)) {
    ...
    new_info <- list(
      order_it = order_it, order_inv = order_inv,
