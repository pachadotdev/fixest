...
#'
#' dict <- c(
#'   "Petal.Length" = "Length (Petal)", "Petal.Width" = "Width (Petal)",
#'   "Sepal.Length" = "Length (Sepal)", "Sepal.Width" = "Width (Sepal)"
#' )
#'
#' setFixest_coefplot(
#'   ci.col = 2, pt.col = "darkblue", ci.lwd = 3,
...
coefplot <- function(object, ..., style = NULL, sd, ci_low, ci_high, df.t = NULL,
  ...
  info <- coefplot_prms(
    object = object, ..., sd = sd, ci_low = ci_low,
    ci_high = ci_high,
  ...
  if (horiz) {
    ...
    if (lab.fit == "auto") {
      ...
      if (any(is_collided) || lab.cex < lab.min.cex) {
        lab.info <- xaxis_labels(
          at = x_at, labels = x_labels, line.max = 1,
        ...
        if (lab.info$failed) {
          # switch to tilted
          lab.fit <- "tilted"
          lab.info <- xaxis_biased(
            at = x_at, labels = x_labels, line.min = LINE_MIN_TILTED,
  ...
  if (!add) {
    ...
    if (!missing(bg)) {
      ...
      if (horiz) {
        rect(
          xleft = my_ylim[1] - dy, ybottom = my_xlim[1] - dx, xright = my_ylim[2] + dy,
          ytop = my_xlim[2] + dx, col = bg
        )
      } else {
        rect(
          xleft = my_xlim[1] - dx, ybottom = my_ylim[1] - dy, xright = my_xlim[2] + dx,
  ...
  if (!is.na(ci.lwd) && ci.lwd > 0) {
    ...
    if (horiz) {
      segments(
        x0 = ci_low, y0 = x, x1 = ci_high, y1 = x, lwd = par_fit(ci.lwd, prms$id),
        col = par_fit(ci.col, prms$id), lty = par_fit(ci.lty, prms$id)
      )
    } else {
      segments(
        x0 = x, y0 = ci_low, x1 = x, y1 = ci_high, lwd = par_fit(ci.lwd, prms$id),
        col = par_fit(ci.col, prms$id), lty = par_fit(ci.lty, prms$id)
      )
    }
    ...
    if (horiz) {
      #  i) ci_high
      segments(
        x0 = ci_high[qui], y0 = x[qui] - ci.width, x1 = ci_high[qui], y1 = x[qui] + ci.width,
        lwd = par_fit(ci.lwd, prms$id[qui]), col = par_fit(ci.col, prms$id[qui]),
        lty = par_fit(ci.lty, prms$id[qui])
      )
      #  ii) ci_low
      segments(
        x0 = ci_low[qui], y0 = x[qui] - ci.width, x1 = ci_low[qui], y1 = x[qui] + ci.width,
        lwd = par_fit(ci.lwd, prms$id[qui]), col = par_fit(ci.col, prms$id[qui]),
        lty = par_fit(ci.lty, prms$id[qui])
      )
    } else {
      #  i) ci_high
      segments(
        x0 = x[qui] - ci.width, y0 = ci_high[qui], x1 = x[qui] + ci.width, y1 = ci_high[qui],
        lwd = par_fit(ci.lwd, prms$id[qui]), col = par_fit(ci.col, prms$id[qui]),
        lty = par_fit(ci.lty, prms$id[qui])
      )
      #  ii) ci_low
      segments(
        x0 = x[qui] - ci.width, y0 = ci_low[qui], x1 = x[qui] + ci.width, y1 = ci_low[qui],
        lwd = par_fit(ci.lwd, prms$id[qui]), col = par_fit(ci.col, prms$id[qui]),
        lty = par_fit(ci.lty, prms$id[qui])
      )
    }
...
coefplot_prms <- function(object, ..., sd, ci_low, ci_high, x, x.shift = 0, dict,
        ...
        if (is.null(df.t)) {
          ...
          if (is.null(df.t)) {
            if (not_too_many_messages("coefplot_df")) {
              message(
                "The degrees of freedom for the t distribution could",
  ...
  return(list(
    prms = prms, num_axis = AXIS_AS_NUM, at = x_at, labels = x_labels,
...
gen_iplot <- function() {
  ...
  iplot_fun <- paste0(
    "iplot = function(object, ..., i.select = 1, ", iplot_args, "){\n\n",
