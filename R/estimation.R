...
feols <- function(fml, data, vcov, weights, offset, subset, split, fsplit, split.keep,
        ...
        for (ii in seq_along(my_lhs)) {
          ...
          for (jj in rhs_group[[j]]) {
            ...
            if (do_iv) {
              ...
              if (isFixef) {
                my_res <- feols(
                  env = current_env, xwx = xwx[qui_X, qui_X, drop = FALSE],
                  xwy = xwy[[ii]][qui_X],
                  X_demean = X_demean[, qui_X, drop = FALSE],
                  y_demean = y_demean[[ii]]
                )
  ...
  if (do_iv) {
    ...
    if (length(pblm) > 0) {
      ...
      stopi(
        "Endogenous variables should not be used as exogenous regressors. ",
  ...
  if (skip_fixef) {
    ...
    if (verbose >= 1) {
      if (length(fixef_sizes) > 1) {
        gt("Demeaning", FALSE)
        cat(" (iter: ", paste0(c(
          tail(res$iterations, 1),
          res$iterations[-length(res$iterations)]
        ), collapse = ", "), ")\n",
        sep = ""
        )
      } else {
  ...
  if (!is.null(dots$resid_1st_stage)) {
    ...
    resid_new <- cpp_iv_resid(
      res$residuals, res$coefficients,
...
feNmlm <- function(fml, data, family = c("poisson", "negbin", "logit", "gaussian"), NL.fml, vcov,
  ...
  opt <- try(stats::nlminb(
    start = start, objective = femlm_ll, env = env, lower = lower,
  ...
  if ("try-error" %in% class(opt)) {
    ...
    warning_msg <- paste0(
      "[", origin, "] Optimization failed at iteration ",
      iter, ". Reason: ", gsub("^[^\n]+\n *(.+\n)", "\\1", opt)
    )
    if (IN_MULTI) {
      stack_multi_notes(warning_msg)
    ...
    warning_msg <- ""
    if (!opt$message %in% c(
      "X-convergence (3)", "relative convergence (4)",
  ...
  coeftable <- data.frame(
    "Estimate" = coef, "Std. Error" = se_format, "z value" = zvalue,
  ...
  if (do_summary) {
    ...
    res <- summary(res,
      vcov = vcov, ssc = ssc, agg = agg, lean = lean,
      summary_flags = summary_flags, nthreads = nthreads
    )
  }
...
warn_fixef_iter <- function(env, stack_multi = FALSE) {
  ...
  if (fixef.iter.limit_reached > 0) {
    goWarning <- TRUE
    warning_msg <- paste0(
      origin, ": [Getting the fixed-effects] iteration limit reached (",
      fixef.iter, ").",
...
warn_step_halving <- function(env, stack_multi = FALSE) {
  ...
  if (nb_sh > 0) {
    msg <- paste0(
      "feglm: Step halving due to non-finite deviance (",
  ...
  for (i in seq_along(lhs)) {
    for (j in seq_along(rhs_sw)) {
      ...
      if (any(is_na_current)) {
        my_env <- reshape_env(env, which(!is_na_current),
          lhs = lhs[[i]],
...
multi_fixef <- function(env, estfun) {
  ...
  for (i in 1:n_fixef) {
    ...
    if (length(all.vars(fml_fixef)) > 0) {
      ...
      assign_fixef_env(
        env, family, origin_type, fixef_id, fixef_sizes,
