...
#' dict <- c(
#'   "Month::5" = "May", "Month::6" = "Jun", "Month::7" = "Jul",
...
#' est_sub1 <- feols(Ozone ~ Solar.R + Temp + Wind,
#'   data = airquality,
#'   subset = ~ Month %in% 5:6
#' )
#' est_sub2 <- feols(Ozone ~ Solar.R + Temp + Wind,
#'   data = airquality,
#'   subset = ~ Month %in% 7:8
#' )
#' est_sub3 <- feols(Ozone ~ Solar.R + Temp + Wind,
#'   data = airquality,
#'   subset = ~ Month == 9
#' )
#'
...
#' etable(est_header, headers = list(
#'   "_Group" = c("A", "A", "B"),
...
#'
#' dict <- c(
#'   "note 1" = "*Notes:* This data is not really random.",
...
gen_etable_aliases <- function() {
  ...
  qui_df <- !arg_name %in% c(
    "tex", "title", "label", "float", "style.tex",
...
results2formattedList <- function(dots, vcov = NULL, ssc = getFixest_ssc(), stage = 2,
  ...
  args_dual <- c(
    "tpt", "arraystretch", "fontsize", "adjustbox",
    "tabular", "signif.code"
  )
  ...
  check_arg(
    "logical scalar", replace, fixef_sizes, fixef_sizes.simplify,
  ...
  if (isTex) {
    ...
    if (!is.null(meta)) {
      ...
      check_set_value(meta_vars, "multi match(time, date, sys, author, comment, call)",
        .message = paste0(
          "The argument 'meta' must be a one-sided formula composed of the following variables: \n",
        ...
        if (isTRUE(meta.author)) {
          ...
          if (length(unique(user_all)) == 1) {
            ...
            if (length(unique(user_all)) == 2) {
              ...
              author_txt <- sma(
                "{author_txt}{user_all[1]} (login), {user_all[2]} (user), ",
  ...
  if (!missnull(vcov) && identical(class(vcov), "list") && length(vcov) > 1) {
    ...
    if (is_rep || n_models == 1) {
      ...
      check_value(vcov, "list len(value)",
        .value = n_models,
  ...
  for (i in seq_along(extralines)) {
    check_value(extralines[[i]], "vector | function | os formula | list",
      .message = sma(
        "The elements of argument 'extralines' must be vectors of ",
    ...
    if ("formula" %in% class(el)) {
      ...
      if (!is.function(el)) {
        ...
        if (length(el) < n_models) {
          ...
          if (n_models %% n_el != 0) {
            stop_up(
              "In 'extralines', the number of elements in the {nth ? i} line ",
  ...
  if (length(pblm) > 0) {
    stop_up(
      "Argument `fitstat` must be a one sided formula (or a character vector) ",
  ...
  for (m in 1:n_models) {
    ...
    if (is.null(a)) {
      a <- data.frame(
        coef = NA_real_, se = NA_real_, tstat = NA_real_,
    ...
    if (TRUE) {
      ...
      if (any(tv > 1)) {
        ...
        stop_up(
          "Problematic value for argument 'dict': The variables {enum.q ? var_pblm}",
  ...
  if (IS_FIXEF_GROUP && length(fe_names) > 0) {
    ...
    if (isTRUE(fixef.group) && length(fe_names) > 1) {
      ...
      if (any(is_inconsistent)) {
        ...
        warn_up(
          "In 'fixef.group', ", msg, deparse_long(fixef.group[[i]]),
  ...
  res <- list(
    se_type_list = se_type_list, var_list = var_list, coef_list = coef_list,
...
etable_internal_latex <- function(info) {
  ...
  if (tabular == "normal") {
    tabular_begin <- paste0(
      "\\begin{tabular}{", space, "l",
      strrep("c", n_models),
      space, "}\n", style$line.top
    )
  ...
  if (length(all_vars) == 0) {
    if ((!is.null(keep) || !is.null(drop)) && length(group) == 0) {
      if (!is.null(keep) && !any(grepl("^%", keep))) {
        msg <- paste0(
          " In particular, to 'keep' variables using their original names ",
          "(before dict is applied), use the special character '%' first. ",
          "E.g. keep = \"%", keep[1], "\""
        )
      } else if (!is.null(drop) && !any(grepl("^%", drop))) {
        msg <- paste0(
          " In particular, to 'drop' variables using their original names ",
          "(before dict is applied), use the special character '%' first. ",
          "E.g. drop = \"%", drop[1], "\""
        )
      } else {
      ...
      stop_up(
        "Not any variable was selected, please reframe your keep/drop arguments.",
  ...
  if (length(fe_names) > 0 && !"fixef" %in% drop.section) {
    ...
    if (fixef_sizes) {
      ...
      if (any(is_complex)) {
        fe_names_nbItems <- paste0(
          style$fixef_sizes.prefix, fe_names_raw[is_complex],
          style$fixef_sizes.suffix
        )
        all_nb_FEs <- cbind(fe_names_nbItems, all_nb_FEs[is_complex, , drop = FALSE])
        nb_FE_lines <- paste0(
          paste0(apply(all_nb_FEs, 1, paste0, collapse = " & "),
            collapse = "\\\\\n"
          ),
          "\\\\\n"
        )
    ...
    FE_lines <- paste0(
      paste0(apply(all_fe, 1, paste0, collapse = " & "),
        collapse = "\\\\\n"
      ),
      "\\\\\n"
    )
  ...
  if (length(slope_names) > 0 && !"slopes" %in% drop.section) {
    ...
    slope_lines <- paste0(
      paste0(apply(all_slopes, 1, paste0, collapse = " & "),
        collapse = "\\\\\n"
      ),
      "\\\\\n"
    )
    ...
    if ("coef" %in% drop.section) {
      ...
      if (isUniqueSD) {
        ...
        if (coefstat == "se") {
          ...
          coefstat_sentence <- paste0(
            " co-variance matrix, ", round(ci * 100),
            "\\% confidence intervals in brackets"
          )
        }
        info_SE_footer <- paste0(
          bottom_line, foot_intro, my_se,
          coefstat_sentence, "}}\\\\\n"
        )

        if (add_signif) {
          info_SE_footer <- paste0(
            info_SE_footer, foot_intro, "Signif. Codes: ",
        ...
        if (add_signif) {
          info_SE_footer <- paste0(
            bottom_line, foot_intro, "Signif. Codes: ",
            paste(names(signif.code), signif.code,
              sep = ": ", collapse = ", "
            ),
            "}}\\\\\n"
  ...
  res <- c(
    meta, table_begin, info_tikz, caption, info_center, info_width, info_font, adj_box_begin,
...
etable_internal_df <- function(info) {
  ...
  if (length(all_vars) == 0) {
    if ((!is.null(keep) || !is.null(drop)) && length(group) == 0) {
      ...
      stop_up(
        "Not any variable was selected, please reframe your keep/drop arguments.", msg,
        "\n To drop the coefficients section, use the argument drop.section = 'coef'."
      )
    }
...
style.tex <- function(main = "base", depvar.title, model.title, model.format, line.top,
  ...
  if ("main" %in% names(mc)) {
    if (main == "base") {
      res <- list(
        depvar.title = "Dependent Variable(s):", model.title = "Model:",
        model.format = "(1)",
      ...
      res <- list(
        depvar.title = "", model.title = "", model.format = "(1)",
        line.top = "\\toprule", line.bottom = "\\bottomrule",
        var.title = "\\midrule",
...
build_tex_png <- function(x, view = FALSE, export = NULL, markdown = NULL,
  ...
  if (do_build) {
    ...
    if (DO_CMD) {
      outcome <- suppressWarnings(system2(
        "pdflatex",
        sma(
          "-halt-on-error -interaction=nonstopmode {draft}",
          " etable.tex"
        ),
    ...
    if (!ok_cmd_magick) {
      ...
      pdftools::pdf_convert("etable.pdf",
...
check_set_path <- function(x, type = "", create = TRUE, up = 0) {
  ...
  if ("r" %in% flags) {
    ...
    stop_up(
      "Argument '", x_dp, "' should be a path to a ", msg,
...
fix_pkgwdown_path <- function() {
  ...
  if (!isTRUE(file.exists("R/alias_VCOV.R"))) {
    return(NULL)
  }
...
style_apply <- function(coef.style, coef_mat, coef_names) {
  ...
  for (i in seq_along(coef.style)) {
    ...
    if (!grepl(":coef(_se)?:", cs_name)) {
      warn_up(
        "In the argument 'coef.style', the names must contain the ",
...
highlight_apply <- function(highlight, coef_mat, coef_names) {
  ...
  for (i in seq_along(highlight)) {
    ...
    if (thick > 6) {
      stop_up(
        "In the argument 'highlight', the name of the {nth?i}",
  ...
  for (i in seq_along(x)) {
    ...
    if (grepl(";", xi, fixed = TRUE)) {
      ...
      if (length(xi_split) != 2) {
        stop_up(
          up = 3, "In the argument {bq?arg_name}, the value of the {nth?i}",
      ...
      if (pool) {
        cell[[length(cell) + 1]] <- expand.grid(
          xi_1_loc$coef:xi_2_loc$coef,
          xi_1_pos:xi_2_pos
        )
      } else {
...
coef_pos_parse <- function(x, coef_names, n_models, i, arg_name) {
  ...
  if (length(x_split) > 2) {
    stop_up(
      "In the argument {bq?arg_name}, the location of the {nth?i}",
...
pmatch_varname <- function(x, coef_names, arg_name) {
  ...
  if (!ok) {
    stop_up(
      up = 5, "In the argument '", arg_name, "', the value '", x,
...
format_se_type_latex <- function(x, dict = c(), inline = FALSE) {
  ...
  main_type_dict <- c(
    "Clustered" = "Clustered", "Two-way" = "Clustered",
...
extralines_extractor <- function(x, name = NULL, tex = FALSE) {
  ...
  if (length(current_vars) > 1 && is_name) {
    stop_up("You cannot give list names in 'extralines' when several values are ",
      ...
      "formula to make it work (concerns '", name, "').",
      up = 2
    )
  }
  ...
  if (!all(current_vars %in% valid_keys)) {
    ...
    stop_up("Argument 'extralines' can be a one-sided formula whose variables refer ",
      ...
      "PROBLEM: the values{$s, enum, were?pblm} not valid.",
      up = 2
    )
  }
...
check_set_adjustbox <- function(adjustbox, up = 0) {
  ...
  if (is.null(adjustbox)) {
    ...
    if (adjustbox > 3) {
      warn_up(
        "When 'adjustbox' is a number, the unit is the text-width. ",
    ...
    if (!is_numeric_in_char(adj_nbr)) {
      stop_up(
        "The number in the argument 'adjustbox' (equal to '", adjustbox,
...
check_set_page_width <- function(page.width, up = 0) {
  ...
  if (isTRUE(attr(page.width, "no-margin"))) {
    ...
    if (!unit %in% c("cm", "mm", "in", "pt", "em", "ex")) {
      stop_up(
        "The argument 'page.width' must represent a 'Latex' measure, ex: 12cm. ",
