  ...
  if (isFALSE(tex) && is_png) {
    ...
    if (is_pp_tex) {
      ...
      if (length(tex_output) > 0) {
        ...
        warning(
          "The argument", enumerate_items(args, "s.don't.quote"),
...
gen_etable_aliases <- function() {
  ...
  esttable_fun <- paste0(
    "esttable = function(", esttable_args, "){\n\n",
...
results2formattedList <- function(dots, vcov = NULL, ssc = getFixest_ssc(), stage = 2,
  ...
  for (i in seq_along(extralines)) {
    check_value(extralines[[i]], "vector | function | os formula | list",
      .message = paste0("The elements of argument 'extralines' must be vectors of length ", n_models, ", logical scalars, functions, one-sided formulas, or lists."),
      .value = n_models
    )
...
setFixest_etable <- function(digits = 4, digits.stats = 5, fitstat,
  ...
  check_arg(
    "logical scalar", se.below, fixef_sizes, fixef_sizes.simplify,
...
build_tex_png <- function(x, view = FALSE, export = NULL, markdown = NULL,
  ...
  if (do_build) {
    ...
    if (DO_CMD) {
      outcome <- system2(
        "pdflatex", .dsb("-halt-on-error -interaction=nonstopmode .[draft] etable.tex"),
    ...
    if (ok_cmd_tex && do_rerun) {
      outcome <- system2(
        "pdflatex", "-halt-on-error -interaction=nonstopmode etable.tex",
        "etable_shell_pdf.log", "etable_shell_pdf.err"
      )
    ...
    if (DO_CMD) {
      outcome <- system2(
        "magick", .dsb("-density 600 etable.pdf -colorspace RGB .[png_name]"),
...
fix_pkgwdown_path <- function() {
  ...
  if (!isTRUE(file.exists("R/VCOV_aliases.R"))) {
    return(NULL)
  }
...
style_apply <- function(coef.style, coef_mat, coef_names) {
  ...
  for (i in seq_along(coef.style)) {
    ...
    if (!grepl(":coef(_se)?:", cs_name)) {
      warn_up(
        "In the argument 'coef.style', the names must contain the ':coef:' (or :coef_se:) string. ",
...
highlight_apply <- function(highlight, coef_mat, coef_names) {
  ...
  for (i in seq_along(highlight)) {
    ...
    if (thick > 6) {
      stop_up(
        "In the argument 'highlight', the name of the ", n_th(i),
  ...
  for (i in seq_along(x)) {
    ...
    if (grepl(";", xi, fixed = TRUE)) {
      ...
      if (length(xi_split) != 2) {
        stop_up(
          up = 3, "In the argument '", arg_name, "', the value of the ", n_th(i),
...
coef_pos_parse <- function(x, coef_names, n_models, i, arg_name) {
  ...
  if (length(x_split) > 2) {
    stop_up(
      "In the argument '", arg_name, "', the location of the ", n_th(i),
