...
#'
#' @seealso
#' [`feols`] for OLS estimation with multiple fixed-effects.
#'
...
#' }
#'
#' # Use macros to grep variables in your data set ----
#'
...
xpd <- function(fml, ..., add = NULL, lhs, rhs, data = NULL, frame = parent.frame()) {
    ...
    .xpd(
        fml = fml, ..., add = add, lhs = lhs, rhs = rhs, data = data, check = TRUE,
...
demean <- function(X, f, slope.vars, slope.flag, data, weights,
    ...
    quf_info_all <- cpppar_quf_table_sum(
...
#'
#' You can set formula macros globally with `setFixest_fml`. These macros can then be used in `fixest` estimations or when using the function [`xpd`][fixest2::setFixest_fml].
#'
