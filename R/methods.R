...
print.fixest <- function(x, n, type = "table", fitstat = NULL, ...) {
  ...
  if (x$method_type == "feNmlm") {
    family_format <- c(
      poisson = "Poisson", negbin = "Negative Binomial",
  ...
  if (isTRUE(x$iv)) {
    ...
    first_line <- sma(
      "TSLS estimation - Dep. Var.: {as.character(x$fml)[[2]]}\n",
...
summary.fixest <- function(object, vcov = NULL, cluster = NULL, ssc = NULL, .vcov = NULL,
    ...
    for (s in seq_along(stage)) {
      if (stage[s] == 1) {
        for (i in seq_along(object$iv_first_stage)) {
          ...
          stage_names[length(stage_names) + 1] <- paste0(
            "First stage: ",
  ...
  if (lean) {
    var2clean <- c(
      "fixef_id", "data", "residuals", "fitted.values", "scores", "sumFE",
...
summary.fixest_list <- function(object, se, cluster, ssc = getFixest_ssc(), .vcov,
  ...
  for (i in seq_along(object)) {
    my_res <- summary(object[[i]],
      se = se, cluster = cluster, ssc = ssc,
  ...
  if (!is.null(object$fixef_terms)) {
    ...
    for (q in seq_along(slope_flag)) {
      ...
      if (sf == 0) {
        ...
        for (j in 0:(nb - 1 - adj)) {
          fixef_values[[i + j + adj]] <- fixef_coef[seq(start + j,
            by = nb,
...
model.matrix.fixest <- function(object, data, type = "rhs", na.rm = TRUE, subset = FALSE,
  ...
  type <- check_set_types(type, c(
    "lhs", "rhs", "fixef", "iv.endo", "iv.inst", "iv.exo",
    "iv.rhs1", "iv.rhs2"
  ))
