...
#' standard-errors, and [`fixef.fixest`] to extract the
...
#'
#' @seealso
#' See also the main estimation functions [`femlm`], [`feols`] or [`feglm`]. Use [`fixef.fixest`] to extract the fixed-effects coefficients.
#'
#' @author
#' Laurent Berge
#'
...
#'
#' @seealso
#' The main estimation functions [`femlm`], [`feols`] or [`feglm`]. Use [`summary.fixest`] to see the results with the appropriate standard-errors, [`fixef.fixest`] to extract the fixed-effect coefficients, and the function to visualize the results of multiple estimations.
#'
#' @author
#' Laurent Berge
#'
...
se.fixest <- function(object, vcov = NULL, ssc = NULL, cluster = NULL,
    ...
    mat <- coeftable(object,
        vcov = vcov, ssc = ssc, cluster = cluster,
...
#'
#' @seealso
#' See also the main estimation functions [`femlm`], [`feols`] or [`feglm`]. Use [`summary.fixest`] to see the results with the appropriate standard-errors, and [`fixef.fixest`] to extract the fixed-effects coefficients.
#'
#' @author
#' Laurent Berge
#'
...
#'
#' @seealso
#' See also the main estimation functions [`femlm`], [`feols`] or [`feglm`]. [`summary.fixest`], [`confint.fixest`], [`vcov.fixest`], [`fixef.fixest`].
#'
#' @examples
#'
...
coef.fixest <- function(object, keep, drop, order,
    ...
    if (is_calling_fun("deltaMethod")) {
        ...
        if ("parameterNames" %in% names(mc_DM)) {
            ...
            check_value(PN, "character vector no na len(data)",
                .data = res,
...
predict.fixest <- function(object, newdata, type = c("response", "link"), se.fit = FALSE,
    ...
    if (length(linear.varnames) > 0) {
        ...
        if (length(varNotHere) > 0) {
            stop(
                "The variable", enumerate_items(varNotHere, "s.quote"),
        ...
        if (!is.null(mm_info_new)) {
            ...
            for (i in seq_along(mm_info)) {
                ...
                if ("coef_names_full" %in% names(mm_i)) {
                    ...
                    if (length(pblm) > 0) {
                        stop(dsb(
                            "In i(), predictions cannot be done for values that were not present at estimation time.",
...
model.matrix.fixest <- function(object, data, type = "rhs", na.rm = TRUE, subset = FALSE,
    ...
    if ("fixef" %in% type) {
        ...
        fixef_df <- error_sender(
            prepare_df(fixef_terms_full$fe_vars, data, fastCombine = FALSE),
...
#'
#' @details
#' Hat values are not available for [`fenegbin`][fixest2::femlm], [`femlm`] and [`feNmlm`] estimations.
#'
