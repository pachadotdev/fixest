          ...
          for (k in seq_along(guesses)) {
            ...
            if (type == "fixef") {
              ...
              if (length(object$fixef_vars) < guesses$fixef) {
                msg <- setNames(
                  nm = sma("the {nth ? guesses$fixef} fixed-effect"),
                  sma(
                    "the estimation had only {Len ? object$fixef_vars} ",
                    "fixed-effect{$s}"
                  )
                )
                err_msg <- c(err_msg, msg)
...
vcov_conley <- function(x, lat = NULL, lon = NULL, cutoff = NULL, pixel = 0,
  ...
  # it makes semi-global changes => the values of the args here are modified
  slide_args(x,
    lat = lat, lon = lon, cutoff = cutoff, pixel = pixel,
...
vcov_setup <- function() {
  ...
  vcov_hetero_setup <- list(
    name = c("hetero", "white", "hc1"),
  ...
  vcov_clust_setup$vars <- list(
    cl1 = list(
      guess_from = list(panel.id = 1, fixef = 1),
  ...
  cl1 <- list(
    guess_from = list(fixef = 1), label = "first cluster",
    to_int = TRUE, rm_nested = TRUE
  )
  cl2 <- list(
    guess_from = list(fixef = 2), label = "second cluster",
    to_int = TRUE, rm_nested = TRUE
  )
  cl3 <- list(
    guess_from = list(fixef = 3), label = "third cluster",
    to_int = TRUE, rm_nested = TRUE
  )
  cl4 <- list(
    guess_from = list(fixef = 4), label = "fourth cluster",
    to_int = TRUE, rm_nested = TRUE
  )

  vcov_twoway_setup <- list(
    name = "twoway",
    fun_name = "vcov_cluster_internal",
    vcov_label = "Clustered"
  )
  ...
  vcov_conley_hac_setup <- list(
    name = c("conley_hac", "hac_conley"),
...
vcov_conley_internal <- function(bread, scores, vars, sandwich, nthreads,
  ...
  id_full <- to_integer(lat_cell, lon_cell,
    sorted = TRUE, add_items = TRUE,
  ...
  meat <- cpp_vcov_conley(scores_ro, lon_ro, lat_ro,
    distance = distance,
