...
femlm_hessian <- function(coef, env) {
  ...
  if (isNL) {
    ...
    z <- numDeriv::genD(evalNLpart, coef[nonlinear.params],
      env = env,
      method.args = hessian.args
    )$D[, -(1:k), drop = FALSE]
  ...
  if (family == "negbin") {
    ...
    h.theta.L <- famFuns$hess.thetaL(
      theta, jacob.mat, y, dxi_dbeta, dxi_dother,
      ll_d2, ll_dx_dother
    )
...
evalNLpart <- function(coef, env) {
  ...
  if (nbMaxSave == 0) {
    ...
    if (anyNA(y_nl)) {
      stop(
        "Evaluation of non-linear part returns NAs. The coefficients were: ",
  ...
  if (family == "logit" && FALSE) {
    ...
    if (!warn_overfit_logit && max(abs(mu_noDum)) >= 100) {
      ...
      if (!isNL || (isLinear && max(abs(mu_L)) >= 100)) {
        ...
        stopi(
          "in femlm(): Likely presence of a separation problem (coef->Inf). ",
...
get_NL_Jacobian <- function(coef, env) {
  ...
  if (anyNA(jacob.mat)) {
    ...
    stopi(
      "ERROR: The Jacobian of the nonlinear part has NA!\n",
...
get_model_null <- function(env, theta.init) {
  ...
  if (family == "poisson") {
    ...
    opt <- nlminb(
      start = theta.guess, objective = famFuns$ll0_theta, y = y,
...
deriv_xi_other <- function(ll_dx_dother, ll_d2, env, coef) {
  ...
  if (Q == 1) {
    ...
    dxi_dother <- cpp_partialDerivative_other(
      iterMax, Q, N,
      epsDeriv = deriv.tol, ll_d2, ll_dx_dother,
...
conv_seq <- function(coef, mu_in, env, iterMax) {
  ...
  if (Q == 2 & family == "poisson") {
    ...
    res <- cpp_conv_seq_poi_2(
      n_i = info$n_i, n_j = info$n_j, n_cells = info$n_cells,
    ...
    res <- cpp_conv_seq_gnl(
      family = family_nb, iterMax = iterMax, diffMax = fixef.tol,
...
dconv_seq <- function(dxi_dbeta, jacob.mat, ll_d2, env, iterMax) {
  ...
  if (Q == 2) {
    ...
    res <- cpp_derivconv_seq_2(
      iterMax = iterMax, diffMax = deriv.tol, n_vars = n_vars,
    ...
    res <- cpp_derivconv_seq_gnl(
      iterMax = iterMax, diffMax = deriv.tol, n_vars,
...
setup_poisson_fixedcost <- function(env) {
  ...
  res <- list(
    n_i = max(dum_A), n_j = max(dum_B), n_cells = n_cells,
