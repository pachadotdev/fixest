.onLoad <- function(libname, pkgname) {
  ...
  options("fixest2_dict" = c("(Intercept)" = "Constant"))
  options("fixest2_notes" = TRUE)
  options("fixest2_print" = list(type = "table"))
...
.onAttach <- function(libname, pkgname) {
  ...
  if (length(msg) > 0) {
    msg <- c("(Permanently remove the following message with fixest2_startup_msg(FALSE).)", msg)
