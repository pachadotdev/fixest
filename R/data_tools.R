...
ref <- function(x, ref) {
  ...
  if (!is.list(ref)) {
    if (is.character(ref[1]) && grepl("^(cut|bin)", ref[1])) {
      ...
      if (!is.numeric(x)) {
        stop(.dsb(
          "To use the special binning `.[ref[1]]` the variable ",
...
n_unik <- function(x) {
  ...
  if (is.vector(x)) {
    ...
    for (i in 1:n_x) {
      x_all[[i]] <- error_sender(
        eval(str2lang(x_all_names[i]), parent.frame()),
      ...
      check_value(x_all[[i]], "data.frame",
        .message = sma(
          "The value {bq?x_all_names[i]} (in the LHS of the formula) ",
  ...
  check_set_value(naked_vars, "multi match",
    .choices = valid_vars,
    .message = sma(
      "The formula must only use variables from",
...
to_integer <- function(..., sorted = FALSE, add_items = FALSE, items.list = FALSE,
  ...
  if (length(unique(n_all)) != 1) {
    stopi(
      "All elements in `...` should be of the same length ",
