  ...
  if (type == "warn") {
    ...
    if (length(x) != length(y)) {
      ...
      if (!all(is.na(x) == is.na(y))) {
        ...
        stop(
          "Position of the NA values differ. ",
        ...
        if (all(qui_pblm)) {
          if (length(x) == 1) {
            stop(
              "Non-NA values differ: ",
              "\nEXPECTED: ", y[!is_na], "\n",
              "\n..ACTUAL: ", x[!is_na]
            )
          } else {
            stop(
              "All non-NA values differ: 1st elem.: ",
              "\nEXPECTED: ", y[!is_na][1], "\n",
              "\n..ACTUAL: ", x[!is_na][1]
            )
          }
...
test_contains <- function(x, pattern) {
  ...
  if (!grepl(pattern, x)) {
    stop(
      "The pattern `", pattern, "` was not found in the following string:",
      "\n\n", x
    )
  }
}
...
test_err_contains <- function(x, pattern) {
  ...
  if (!inherits(err, "try-error")) {
    ...
    stop(
      "The expression should lead to an error.",
  ...
  if (!grepl(pattern, err)) {
    stop(
      "The pattern `", pattern, "` was not found in the following error message:",
      "\n\n", err
    )
  }
}
...
check_set_options <- function(x, options, op = NULL, free = FALSE, case = FALSE) {
  ...
  for (i in 1:n) {
    ...
    if (is.na(pm) && !free) {
      # absence of a match
      stop(
        "The option `", v, "` is not valid for the current operation.\n",
