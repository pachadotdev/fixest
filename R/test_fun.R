...
test <- function(x, y, type = "=", tol = 1e-6) {
  ...
  if (type == "err") {
    ...
    if (length(x) != length(y)) {
      stop(
        "Lengths differ: EXPECTED: ", length(y), "\n",
        "                    ACTUAL: ", length(x)
      )
    } else if (anyNA(x)) {
      if (!all(is.na(x) == is.na(y))) {
        if (sum(is.na(x)) != sum(is.na(y))) {
          stop(
            "Number of NA values differ: EXPECTED: ", sum(is.na(y)), "\n",
            "                                ACTUAL: ", sum(is.na(x))
          )
        }
        ...
        stop(
          "Position of the NA values differ. EXPECTED: ", info[na_y], " in position ", i, "\n",
          "                                      ACTUAL: ", info[3 - na_y], "."
        )
        ...
        if (all(qui_pblm)) {
          if (length(x) == 1) {
            stop(
              "Non-NA values differ: EXPECTED: ", y[!is_na], "\n",
              "                          ACTUAL: ", x[!is_na]
            )
          } else {
            stop(
              "All non-NA values differ: 1st elem.: EXPECTED: ", y[!is_na][1], "\n",
              "                                         ACTUAL: ", x[!is_na][1]
            )
          }
        } else {
      ...
      if (all(qui_pblm)) {
        if (length(x) == 1) {
          stop(
            "Values differ: EXPECTED: ", y, "\n",
            "                   ACTUAL: ", x
          )
        } else {
          stop(
            "All values differ: 1st elem.: EXPECTED: ", y[1], "\n",
            "                                  ACTUAL: ", x[1]
          )
        }
      } else {
...
non_ascii <- function(folder = "R") {
  ...
  if (n == 0) {
    return("No non-ASCII character found.")
  }
...
open_all <- function() {
  ...
  R_not_open <- c(
    "R/alias_generator.R", "R/RcppExports.R", "R/Deprecated_funs.R", "R/index",
  ...
  extra_files <- c(
    "../PROBLEMS.R", "../Problems.Rmd", "../Development.R",
