...
set_index_multi <- function(x, index_names) {
  ...
  if (!arg %in% names(index_names)) {
    if (!(identical(x, 1) || identical(x, 0) || identical(x, TRUE))) {
      stopi(
        "The index {bq?arg} is not valid for this list of results (the valid one{$s, are, enum.bq ? names(index_names)}). ",
  ...
  if (is.logical(x)) {
    ...
    if (length(res) == 0) {
      stop_up(
        "The set of regular expressions (equal to: {Q?x}) in {bq?arg} ",
        "didn't match any choice."
      )
    }
  } else if (any(abs(x) > vmax)) {
    stop_up(
      "The index '", arg, "' cannot be greater than ", vmax,
...
n_models <- function(x, lhs = FALSE, rhs = FALSE, sample = FALSE,
  ...
  if (length(dimension) > 1) {
    stopi(
      "You can request the number of different models on **only one** dimension. Currently ",
  ...
  if (is.null(est_1$cov.scaled) || !isTRUE(dots$fromPrint)) {
    for (i in 1:length(object)) {
      object[[i]] <- summary(object[[i]],
        vcov = vcov, se = se, cluster = cluster, ssc = ssc,
...
print.fixest_multi <- function(x, ...) {
  ...
  dict_title <- c(
    "sample" = "Sample", "lhs" = "Dep. var.", "rhs" = "Expl. vars.",
...
coef.fixest_multi <- function(object, keep, drop, order, collin = FALSE,
  ...
  if (long) {
    ...
    res <- cbind(mod_long,
      coefficient = rep(all_names, NROW(res)),
      estimate = res_long
    )
...
coeftable.fixest_multi <- function(object, vcov = NULL, keep = NULL, drop = NULL,
  ...
  if (!isFALSE(wide)) {
    ...
    for (i_select in which(roots %in% wide)) {
      ...
      res_wide <- matrix(value[new_key],
        nrow = length(all_id), ncol = length(all_coef),
...
se.fixest_multi <- function(object, vcov = NULL, keep = NULL, drop = NULL,
  ...
  res <- coeftable(object,
    vcov = vcov, keep = keep, drop = drop,
...
confint.fixest_multi <- function(object, parm, level = 0.95, vcov = NULL, se = NULL,
  ...
  for (i in 1:n) {
    confint_all[[i]] <- confint(object[[i]],
