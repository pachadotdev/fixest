...
compute_cluster_coef_r <- function(family, nb_coef, theta, diffMax_NR, r_mu, r_lhs, r_sum_y, r_dum, r_obsCluster, r_table, r_cumtable, nthreads = 1L) {
  compute_cluster_coef_r_(parse_int(family), parse_int(nb_coef), theta, diffMax_NR, r_mu, r_lhs, r_sum_y, r_dum, r_obsCluster, r_table, r_cumtable, parse_int(nthreads))
}
...
cpp_derivconv_seq <- function(iterMax, diffMax, n_vars, nb_cluster_all, n_cells, index_i, index_j, order, ll_d2, jacob_vector, deriv_init_vector, dum_vector) {
  cpp_derivconv_seq_(parse_int(iterMax), diffMax, n_vars, nb_cluster_all, parse_int(n_cells), parse_int(index_i), parse_int(index_j), order, ll_d2, jacob_vector, deriv_init_vector, dum_vector)
}
...
cpp_poisson_linkinv <- function(x, nthreads = 1L) {
  cpp_poisson_linkinv_(x, parse_int(parse_int(nthreads)))
}
