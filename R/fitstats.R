        ...
        if (is.null(names(alias))) {
            if (length(alias) == 1) {
                ...
                if (IS_SUB) {
                    ...
                    stop(
                        "To define the aliases, please use a named character vector, with the names being the codes, and the values the aliases. \n",
...
fitstat <- function(x, type, simplify = FALSE, verbose = TRUE, show_types = FALSE,
    ...
    if (isTRUE(dots$give_types) || isTRUE(show_types)) {
        ...
        # F-test etc
        comp_types <- c(
            "f", "wf", "ivf", "ivf1", "ivf2", "wald", "ivwald",
...
kp_stat <- function(x) {
    ...
    if (identical(VCOV_TYPE, "IID")) {
        ...
        my_scores <- do.call(cbind, lapply(seq_len(ncol(X_proj)), function(i) Z_proj * X_proj[, i]))
