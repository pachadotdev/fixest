    ...
    if (length(v) == 1) {
      ...
      for (k in seq_along(v)) {
        ...
        if (subtype == "stat") {
          ...
          arg_list[[k]] <- paste0(
            "on ", numberFormatNormal(v$df1), " and ",
    ...
    if (is.null(names(alias))) {
      if (length(alias) == 1) {
        ...
        if (IS_SUB) {
          ...
          stopi(
            "To define the aliases, please use a named character vector, ",
...
fitstat <- function(x, type, simplify = FALSE, verbose = TRUE, show_types = FALSE,
                    frame = parent.frame(), ...) {
  r2_types <- c(
    "sq.cor", "cor2", "r2", "ar2", "pr2", "apr2", "par2", "wr2",
  ...
  if (isTRUE(dots$give_types) || isTRUE(show_types)) {
    ...
    # F-test etc
    comp_types <- c(
      "f", "wf", "ivf", "ivf1", "ivf2", "ivfall", "wald", "ivwald",
    ...
    comp_alias <- c(
      f = "F-test", wf = "F-test (projected)", ivfall = "F-test (IV only)",
    ...
    full_comp_alias <- setNames(paste0(
      comp_alias, ", ",
      rep(c("stat.", "p-value"),
        each = length(comp_alias)
      )
    ), my_names)
    ...
    # "Regular" types
    valid_types <- c(
      "n", "ll", "aic", "bic", "rmse", "g", "my", "theta", r2_types,
      comp_types, full_comp_types, user_types
    )

    tex_alias <- c(
      n = "Observations", ll = "Log-Likelihood", aic = "AIC", bic = "BIC",
    ...
    # add r2 type alias
    type_alias <- c(
      ivf = "ivf1", ivf.stat = "ivf1.stat", ivf.p = "ivf1.p", ivwald = "ivwald1",
    ...
    res <- list(
      types = valid_types, tex_alias = tex_alias, R_alias = R_alias,
...
r2 <- function(x, type = "all", full_names = FALSE) {
  ...
  dict_names <- c(
    "cor2" = "Squared Correlation", "r2" = "R2", "ar2" = "Adjusted R2",
  ...
  types_alias <- c(
    par2 = "apr2", awr2 = "war2", pwr2 = "wpr2", wpar2 = "wapr2",
  ...
  for (i in seq_along(type_all)) {
    ...
    if (within && isFixef) {
      ...
      if (is.null(x$ssr_fe_only) && !is.null(x$fixef_vars)) {
        ...
        res_fe <- feglm(
          fml = new_fml, data = newdata, glm.tol = 1e-2,
