...
collinearity <- function(x, verbose) {
  ...
  if (isSlope) {
    ...
    if (any(!slope_vars_unik %in% names(data))) {
      ...
      stopi(
        "To check collinearity, we need to fetch some variables in the original dataset (",
  ...
  if (isLinear && length(linearVars) >= 2) {
    ...
    for (v in linearVars) {
      ...
      res <- ols_fit(
        y = mat_base[, i], X = mat_base[, -i, drop = FALSE],
  ...
  if (isNL && (length(coef) >= 2 || isFixef)) {
    ...
    for (var in coef_name) {
      ...
      if (var %in% L_coef) {
        ...
        res <- feNmlm(fml,
          data = data, family = "gaussian", NL.fml = NL_fml,
        ...
        if (length(NL_coef) == 1) {
          ...
          res <- feNmlm(fml,
            data = data, family = "gaussian", NL.fml = NL_fml_new,
...
did_means <- function(fml, base, treat_var, post_var, tex = FALSE, treat_dict,
  ...
  if (!usePost) {
    # the simple case
    res <- .meanDiffTable(mat_vars, treat_var, treat_cases, indiv_var,
      raw = raw,
      diff.inv = diff.inv
    )
  ...
  if (tex || !missing(file)) {
    ...
    if (!usePost) {
      ...
      # the treat post line
      cat(
        " & ", "\\multicolumn{4}{c}{", prepostnames[1], "} & \\multicolumn{4}{c}{",
...
xpd <- function(fml, ..., add = NULL, lhs, rhs, data = NULL, frame = parent.frame()) {
  ...
  .xpd(
    fml = fml, ..., add = add, lhs = lhs, rhs = rhs, data = data,
...
demeaning_algo <- function(extraProj = 0, iter_warmup = 15, iter_projAfterAcc = 40,
  ...
  if (internal) {
    return(list(
      extraProj = extraProj, iter_warmup = iter_warmup,
    ...
    if (check) {
      ...
      if (length(value_raw)) {
        ...
        my_call <- error_sender(str2lang(value_raw), "The value '", value_raw,
          "' does not lead to a valid formula: ",
          up = 2
        )
...
dsb_check_set_agg <- function(...) {
  ...
  if (...length() > 1) {
    ...
    stop_up(
      "You cannot have more than two elements in between .[]. Currently there are ",
      ...length() + 1, "."
    )
  }
...
prepare_matrix <- function(fml, base, fake_intercept = FALSE) {
  ...
  if (any(lengths(data_list) != nrow(base)) || any(qui_i)) {
    ...
    if (length(qui_pblm) > 0) {
      ...
      stopi(
        "Evaluation of {bq?all_var_names[qui_pblm]}, returns an object {reason}",
...
fixef_terms <- function(fml, stepwise = FALSE, origin_type = "feols") {
  ...
  if (any(grepl("]", my_vars, fixed = TRUE))) {
    ...
    if (length(var2check_double) > 0) {
      ...
      if (any(qui)) {
        ...
        msg <- sma(
          "Square bracket are special characters used **only** to designate ",
  ...
  if (any(grepl("^i\\(", my_vars))) {
    ...
    if (n_var == 1) {
      ...
      msg <- sma(
        "To interact fixed-effects, use the syntax fe1^fe2 ",
...
fml_combine <- function(fml_char, fastCombine, vars = FALSE) {
  ...
  labels <- attr(
    terms(.xpd(rhs = gsub("\\^(?=[^0-9])", "%^%", fml_char, perl = TRUE))),
    "term.labels"
  )
...
check_set_nthreads <- function(nthreads) {
  ...
  if (nthreads == 0) {
    ...
    if (max_threads == 0) {
      ...
      warn_up(
        "Asked for ", nthreads, " threads while the maximum is ", max_threads,
...
items_to_drop <- function(items, x, varname, keep = FALSE, argname,
  ...
  if (inherits(x, "formula")) {
    ...
    if (length(x) > 2) {
      stop_up(
        "In argument `", argname, "`, if a formula, it must be one-sided. Problem: `",
...
format_number <- function(x, digits = 4, round = FALSE, pow_above = 10,
  ...
  sapply(x, format_nber_single,
    digits = digits, round = round, pow_above = pow_above,
...
dict_apply <- function(x, dict = NULL) {
  ...
  if (any(grepl(" ", names(dict), fixed = TRUE))) {
    ...
    if (anyDuplicated(names(dict))) {
      ...
      stop(
        "The dictionary `dict` contains several items with the same names, it concerns ",
...
show_vars_limited_width <- function(charVect, nbChars = 60, addS = FALSE) {
  ...
  text <- paste0(
    text_s, paste0(charVect[1:qui], collapse = ", "),
...
extract_fe_slope <- function(t) {
  ...
  list(
    fixef_vars = fixef_vars, slope_vars = slope_vars, slope_fe = slope_fe,
...
as.dict <- function(x) {
  ...
  if (anyDuplicated(dict_names)) {
    ...
    stop(
      "The names contain duplicate entries: this is not allowed. The value ",
...
setFixest_dict <- function(dict = NULL, ..., reset = FALSE) {
  ...
  check_arg(..., "dotnames character scalar",
    .message = sma(
      "In '...', each argument must be named. ",
